...
/* an SQL query to enroll a student in a course.*/
insert into enrollments (enrollment_id, student_id, course_id, enrollment_date)
values (12, 5, 205, '2024-06-01'); 
/*Update the email address of a specific teacher in the "Teacher" table*/
update teacher
set email = 'david.lee_updated@school.com'
where teacher_id = 3;
/*an SQL query to delete a specific enrollment record from the "Enrollments" table  based on the student and course*/
...
/*t3*/
/*to calculate the total payments made by a specific student*/
...
/* to retrieve a list of courses along with the count of students enrolled in each 
course*/
...
/*SQL query to find the names of students who have not enrolled in any course*/
select s.first_name, s.last_name
from students s
left join enrollments e on s.student_id = e.student_id
where e.enrollment_id is null;

/* to retrieve the first name, last name of students, and the names of the 
courses they are enrolled in.use join all 3*/
select s.first_name, s.last_name, c.course_name
from students s
join enrollments e on s.student_id = e.student_id
join courses c on e.course_id = c.course_id;

/* to list the names of teachers and the courses they are assigned to*/
select t.first_name, t.last_name, c.course_name
from teacher t
join courses c on t.teacher_id = c.teacher_id;

/*list of students and their enrollment dates for a specific course use aLL 3*/
...
/*names of students who have not made any payments*/
select s.first_name, s.last_name
from students s
left join payments p on s.student_id = p.student_id
where p.payment_id is null;

/*to identify courses that have no enrollments.*/
select c.course_name
from courses c
left join enrollments e on c.course_id = e.course_id
where e.enrollment_id is null;

/* students who are enrolled in more than one course using self join--compaering rows same table*/
...
/*t4*/
/* to calculate the average number of students enrolled in each course*/
...
/*a list of courses with the highest number of enrollment*/
...
/* the total payments made to courses taught by each teacher*/
...
from teacher t;
/*students who are enrolled in all available courses*/
...
/*the names of teachers who have not been assigned to any course*/
...
/*y courses with no enrollments*/
select course_name
from courses
where course_id not in (
  select course_id from enrollments
);
/* the total payments made by each student for each course they are enrolled in.*/
...
/* to calculate the total payments made by each student*/
...
/*a list of course names along with the count of students enrolled in each course.*/
select c.course_name, count(e.student_id) as student_count
from courses c
join enrollments e on c.course_id = e.course_id
group by c.course_name;

/* the average payment amount made by students.*/
